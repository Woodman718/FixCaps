...
class FixCapsNet(nn.Module):
    def __init__(self,conv_inputs,conv_outputs,
                 ...
                 num_classes=7,mode="DS",init_weights=False):
        super().__init__()
...
#             layers += [nn.BatchNorm2d(f_channels,affine=True)]
            ...
            layers += [nn.Conv2d(f_channels,out_c,1, stride=2, bias=False)]
        elif v == "C":           
            layers += [nn.Conv2d(f_channels, f_channels, kernel_size=1, stride=2, bias=False)]
            layers += [LayerNorm(f_channels, eps=1e-6, data_format="channels_first")]
#             layers += [nn.Conv2d(f_channels,f_channels,4,stride=4,)]
        else:
            layers += [nn.Conv2d(f_channels, v, 31,stride=2,groups=g, bias=False)]
            f_channels = v
    return nn.Sequential(*layers)

cfgs= {
    "DS": [3,'N','B','R','F'],#96.135% ,g = 3, using trick can up to 96.256%
    "256"  : [256,'R','F'],#90.942%, g = 1,
    "128"  : [128,'R','F'],#96.498%, g = 1,
    "64"  : [64,'R','F'],#94.32%, g = 1,
    "DS2": ["C",3,'N','B','R','F'],#95.53% ,g = 3
}
