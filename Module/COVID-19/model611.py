...
class Conv_CBAM(nn.Module):
    ...
    def __init__(self, c1, c2, k=1, s=1, p=None, g=1, act=True):
        ...
        self.bn = LayerNorm(c2, eps=1e-6, data_format="channels_first")#nn.BatchNorm2d(c2)
...
def make_features(cfg: list,f_c,out_c=None,g=1,step=2):
    ...
    for v in cfg:
        if v == "M":
            ...
            layers += [nn.Conv2d(f_channels,out_c,1,stride=step, bias=False)]
        elif v == "C":
            layers += [nn.Conv2d(f_channels,f_channels,3,stride=step, bias=False)]
        else:
            layers += [nn.Conv2d(f_channels, v, 31,stride=step,groups=g, bias=False)]
            f_channels = v
    return nn.Sequential(*layers)

cfgs= {
    "DS": [3,'N','B','R','F'],#96.135-96.256% ,g = 3,  primary_unit_size = 16 * 6 * 6 
    "DS2": ["C",3,'N','B','R','F'],#95.53% ,g = 3,  primary_unit_size = 16 * 6 * 6 
